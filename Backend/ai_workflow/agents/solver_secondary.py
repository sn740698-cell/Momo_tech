"""
Agent 7 — Secondary Solver / Reviewer (Solver B).
Independently reviews and solves the task using the same grounded evidence.
Challenges assumptions, catches corner cases, and maintains analytical independence.
"""
import logging
...
from ai_workflow.prompts.solver_prompts import SOLVER_SECONDARY_SYSTEM_PROMPT, SOLVER_SECONDARY_USER_PROMPT
from ai_workflow.services.llm_factory import LLMFactory

logger = logging.getLogger(__name__)


class SecondarySolverAgent(BaseWorkflowAgent):
    """
    Secondary solver providing an independent, critical perspective (Solver B).
    """

    def __init__(self):
        super().__init__(name="secondary_solver_agent")

    async def execute(self, state: WorkflowState) -> Dict[str, Any]:
        provider = LLMFactory.get_provider(role="solver_b")
        ...
        prompt = SOLVER_SECONDARY_USER_PROMPT.format(
        ...
        output = await provider.generate_async(
            prompt=prompt,
            system_prompt=SOLVER_SECONDARY_SYSTEM_PROMPT,
            temperature=0.4
        )

        logger.info("SecondarySolverAgent completed execution.")
        return {"solver_b_output": output.strip()}
