...
class ResponseParser:
    ...
    def clean_text(cls, text: str) -> str:
        ...
        # Deduplicate repeating bullet points generated by LLMs
        ...
        if any(line.strip().startswith(("•", "-", "*")) for line in lines):
            ...
            for line in lines:
                ...
                if s_line.startswith(("•", "-", "*")):
                    # Normalize double bullet markers like "• • " into single "• "
                    s_line = re.sub(r'^(?:[•\-\*]\s*)+', '• ', s_line)
                    core = re.sub(r'^[•\-\*]\s*', '', s_line).strip().lower()
                    if core in seen_bullets:
                        continue
                    seen_bullets.add(core)
                    deduped_lines.append(s_line)
                else:
